...
export default function Inicio({ cliente, contratos, paneles, onGoTo, onMenuClick, onNotifClick, totalNotifs = 0, isAdmin, adminNombre }: Props) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background: HEADER }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ padding:"calc(18px + env(safe-area-inset-top)) 24px 48px", flexShrink:0, background:headerBg }}>
        {/* Logo + menú + campana */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", position:"relative", marginBottom:28 }}>
          ...
          <img src="/logo-player.png" alt="Vista360 Player" className="inicio-logo" style={{ height:42, maxWidth:"68%", objectFit:"contain" }} />
        ...
        {/* Saludo */}
        <div style={{ fontSize:16, color:"rgba(255,255,255,0.86)", marginBottom:4 }}>{saludo}</div>
        <div style={{ fontSize:30, fontWeight:800, color:"#fff", marginBottom:18, letterSpacing:0, lineHeight:1.05 }}>
          {isAdmin ? "Hola Admin" : nombre}
        </div>
        {/* Pill */}
        <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(255,255,255,0.13)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:24, padding:"8px 14px", boxShadow:"0 12px 28px rgba(0,0,0,0.18)" }}>
          <div style={{ width:10, height:10, borderRadius:"50%", background:"#22C55E" }} />
          <span style={{ fontSize:14, color:"#fff", fontWeight:650 }}>{todoOk ? "Todo funcionando" : "Revisa tus campañas"}</span>
        </div>
      </div>

      {/* ── FONDO BLANCO CON CURVA ── */}
      <div style={{ flex:1, overflowY:"auto", background:"#F8FAFD", borderRadius:"28px 28px 0 0", marginTop:-28, padding:"20px 18px 14px", WebkitOverflowScrolling:"touch" as any, overscrollBehavior:"contain" }}>

        {/* RESUMEN GENERAL — título suelto, cards individuales */}
        <div style={{ fontSize:18, fontWeight:800, color:"#08122B", marginBottom:14 }}>Resumen general</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:22 }}>
          {[
            { bg:"#DBEAFE", label:"Campañas activas", val:String(activas.length),
              icon:<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-5"/></svg> },
            { bg:"#DCFCE7", label:"Pantallas activas", val:String(pantallasActivas), onClick: () => onGoTo("mispantallas"),
              icon:<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg> },
            { bg:"#EDE9FE", label:"Última evidencia", val:ultima ? ultima.foto.fecha : "—",
              icon:<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#7C3AED" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg> },
            { bg:"#FFEDD5", label:"Próximo vencimiento", val:proxVenc ? proxVenc.fin : "—",
              icon:<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#EA580C" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="16" y1="2" x2="16" y2="6"/></svg> },
            ...(!isAdmin ? [{
            ...
            <div
              key={i}
              onClick={k.onClick}
              style={{ background:"#fff", border:"1px solid #E8EDF5", borderRadius:14, padding:"13px 12px", minHeight:88, display:"flex", alignItems:"center", gap:11, boxShadow:"0 8px 18px rgba(15,23,42,0.045)", cursor: k.onClick ? "pointer" : "default" }}
            >
              <div style={{ width:44, height:44, borderRadius:22, background:k.bg, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                {k.icon}
              </div>
              <div style={{ minWidth:0 }}>
                <div style={{ fontSize: 13, color:"#111827", marginBottom:5, lineHeight:1.18 }}>{k.label}</div>
                <div style={{ fontSize:19, fontWeight:800, color:"#08122B", lineHeight:1.1 }}>{k.val}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ height:1, background:"#E6EAF1", marginBottom:18 }} />
        <div style={{ fontSize:18, fontWeight:800, color:"#08122B", marginBottom:14 }}>Accesos rápidos</div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:22 }}>
          {[
            { bg:"#FFFFFF", label:"Mis campañas", tab:"campanas" as const,
              icon:<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M3 11v3a2 2 0 0 0 2 2h2l6 4V5L7 9H5a2 2 0 0 0-2 2z"/><path d="M16 9a4 4 0 0 1 0 6"/></svg> },
            { bg:"#FFFFFF", label:"Mis pantallas", tab:"mispantallas" as const,
              icon:<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="13" rx="2"/><path d="M8 21h8M12 17v4"/></svg> },
            { bg:"#FFFFFF", label:"Reportes", tab:"reportes" as const,
              icon:<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#7C3AED" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="7" x2="19" y2="7"/><line x1="5" y1="12" x2="19" y2="12"/><line x1="5" y1="17" x2="19" y2="17"/></svg> },
            { bg:"#FFFFFF", label:"Nueva campaña", tab:"nueva" as const,
              icon:<svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="#EA580C" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> },
          ].map(q => (
            <div key={q.tab} onClick={() => onGoTo(q.tab)} style={{ minHeight:92, background:q.bg, border:"1px solid #E8EDF5", borderRadius:14, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:10, cursor:"pointer", WebkitTapHighlightColor:"transparent", boxShadow:"0 8px 18px rgba(15,23,42,0.035)" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:34 }}>
                {q.icon}
              </div>
              <span style={{ fontSize: 13, color:"#08122B", fontWeight:650, textAlign:"center", lineHeight:1.18 }}>{q.label}</span>
            </div>
          ))}
        </div>
