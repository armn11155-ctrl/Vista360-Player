...
const TABS: { id: Tab; label: string; getIcon: (a: boolean) => React.ReactNode }[] = [
  { id:"inicio", label:"Inicio", getIcon: a =>
    <svg width="22" height="22" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"
      fill={a ? "#0877FF" : "none"} stroke={a ? "#0877FF" : "#9CA3AF"} strokeWidth="1.8">
  ...
  { id:"campanas", label:"Campañas", getIcon: a =>
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? "#0877FF" : "#9CA3AF"} strokeWidth="1.8" strokeLinecap="round">
      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>
    </svg>
  },
  { id:"cobertura", label:"Cobertura", getIcon: a =>
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={a ? "#0877FF" : "#9CA3AF"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  { id:"perfil", label:"Perfil", getIcon: a =>
    <PersonIcon size={22} color={a ? "#0877FF" : "#9CA3AF"} />
  },
];
      ...
      {TABS.map(tab => {
        ...
        return (
          <button key={tab.id} onClick={() => onChange(tab.id)} style={{
            ...
            {tab.getIcon(a)}
            <span style={{ fontSize: 11, fontWeight: a ? 700 : 400, color: a ? "#0877FF" : "#9CA3AF", letterSpacing:0 }}>
              {tab.label}
            </span>
          </button>
        );
